# Similar to scraper.py (stocks), but for crypto currencies
...
date_end = "2023-12-04 00:00:00"
url_root = "https://query1.finance.yahoo.com/v7/finance/download"
script_path = os.path.dirname(os.path.realpath(__file__))
index_path = "data/scraped/yahoo/crypto/index.json"
stop_path = os.path.join(script_path,
                         "stop")  # if this file exists, stop scraping
output_path_root = "data/scraped/yahoo"
csv_path_root = os.path.join(*[output_path_root, "crypto", "csv"])
time_format = "%Y-%m-%dT%H:%M:%S.%f%z"
symbols = []
fails = []
logger = Logger({
    "filepath":
        os.path.join(*[output_path_root, "logs", "scraper_crypto.log"]),
    "level":
        "DEBUG",
})
...
def set_symbols(use_queue: bool = True,
	...
	index = load_index()
	logger.info("Getting symbols from 'urls' key...")
	urls = index["urls"]
	for url in urls:
